...
internal static class LobbyPatcher
{
    ...
    /// <summary>
    /// Computes the diff between two lobbies and returns the changes.
    /// </summary>
    /// <param name="lobby1">The first lobby.</param>
    /// <param name="lobby2">The second lobby.</param>
    /// <returns>The changes between the two lobbies <paramref name="lobby1"/> and <paramref name="lobby2"/></returns>
    /// <remarks>
    /// These are some of the cases that should be handled.
    /// <code>
    /// Case #1
    /// ╔══════════════════╗  ╔═════════════════════════════╗
    /// ║     Old Lobby    ║  ║          New Lobby          ║
    /// ╠═══════╦══════════╣  ╠═══════╦══════════╦══════════╣
    /// ║ ID    ║ Player B ║  ║ ID    ║ Player A ║ Player B ║
    /// ╠═══════╬══════════╣  ╠═══════╬══════════╬══════════╣
    /// ║ Index ║     0    ║  ║ Index ║     0    ║     1    ║
    /// ╚═══════╩══════════╝  ╚═══════╩══════════╩══════════╝
    /// Case #2
    ...
    /// ║     Old Lobby    ║  ║          New Lobby          ║
    /// ╠═══════╦══════════╣  ╠═══════╦══════════╦══════════╣
    /// ║ ID    ║ Player B ║  ║ ID    ║ Player B ║ Player A ║
    /// ╠═══════╬══════════╣  ╠═══════╬══════════╬══════════╣
    /// ║ Index ║     0    ║  ║ Index ║     0    ║     1    ║
    /// ╚═══════╩══════════╝  ╚═══════╩══════════╩══════════╝
    /// Case #3
    ...
    /// ║          Old Lobby          ║  ║     New Lobby    ║
    /// ╠═══════╦══════════╦══════════╣  ╠═══════╦══════════╣
    /// ║ ID    ║ Player A ║ Player B ║  ║ ID    ║ Player B ║
    /// ╠═══════╬══════════╬══════════╣  ╠═══════╬══════════╣
    /// ║ Index ║     0    ║     1    ║  ║ Index ║     0    ║
    /// ╚═══════╩══════════╩══════════╝  ╚═══════╩══════════╝
    /// Case #4
    ...
    /// ║          Old Lobby          ║  ║                New Lobby               ║
    /// ╠═══════╦══════════╦══════════╣  ╠═══════╦══════════╦══════════╦══════════╣
    /// ║ ID    ║ Player A ║ Player B ║  ║   ID  ║ Player A ║ Player B ║ Player C ║
    /// ╠═══════╬══════════╬══════════╣  ╠═══════╬══════════╬══════════╬══════════╣
    /// ║ Index ║     0    ║     1    ║  ║ Index ║     0    ║     1    ║     2    ║
    /// ╚═══════╩══════════╩══════════╝  ╚═══════╩══════════╩══════════╩══════════╝
    /// </code>
    /// </remarks>
    internal static LobbyPatcherChanges GetLobbyDiff(Lobby lobby1, Lobby lobby2)
    {
        ...
        oldLobby.Players ??= new();
        newLobby.Players ??= new();
        ...
        if (oldLobbyPlayersIdxById.Count != 0)
        {
            ...
            for (int playerIdx = oldLobby.Players.Count - 1; playerIdx >= 0; playerIdx--)
            {
                ...
                // If player is not in the new lobby
                // or if its index is greater than in the old lobby meaning that the player joined again.
                if (!newLobbyPlayersIdxById.ContainsKey(oldLobbyPlayer.Id)
                    || newLobbyPlayersIdxById[oldLobbyPlayer.Id] > oldLobbyPlayersIdxById[oldLobbyPlayer.Id])
                {
        ...
        foreach (var newLobbyPlayerId in newLobbyPlayersIdxById.Keys)
        {
            var newLobbyPlayerIdx = newLobbyPlayersIdxById[newLobbyPlayerId];

            // If the player was not in the previous version of the lobby then
            // it means the player joined.
            var wasPlayerInPreviousLobby =
                oldLobbyPlayersIdxById.TryGetValue(newLobbyPlayerId, out var oldLobbyPlayerIdx);
            if (!wasPlayerInPreviousLobby)
            {
                changes.PlayerJoinedChange(newLobbyPlayerIdx, newLobby.Players[newLobbyPlayerIdx]);
                continue;
            }

            // If the player is still at the same place, don't do anything.
            if (newLobbyPlayerIdx < oldLobby.Players.Count &&
                oldLobby.Players[newLobbyPlayerIdx].Id == newLobby.Players[newLobbyPlayerIdx].Id)
            {
                continue;
            }

            // If the player index in the old lobby is greater than its index in
            // the new lobby it means the player never left (and its index
            // likely shifted when other players left).
            // See case #3 from the remarks XML documentation.
            if (oldLobbyPlayerIdx >  newLobbyPlayerIdx)
            {
                continue;
            }

            // If the player index in the new lobby is greater than its index in
            // the old lobby it means the player joined again (and it got added
            // at the end of the player list).
            changes.PlayerJoinedChange(newLobbyPlayerIdx, newLobby.Players[newLobbyPlayerIdx]);
        }
    ...
    private static void ParseAddPatch(LobbyPatch patch, LobbyPatcherChanges changes)
    {
        ...
        else
        {
            switch (patch.path)
            {
                ...
                default: Logger.LogVerbose($"Not implemented add patch with path[{patch.path}]"); break;
            }
        }
    }
    ...
    private static void ParseReplacePatch(LobbyPatch patch, LobbyPatcherChanges changes)
    {
        ...
        else
        {
            switch (patch.path)
            {
                ...
                case "/lastUpdated":
                { changes.LastUpdatedChange((DateTime)patch.value); break; }
                default: Logger.LogVerbose($"Not implemented replace patch with path[{patch.path}]"); break;
            }
        }
    }
    ...
    private static void ParseRemovePatch(LobbyPatch patch, LobbyPatcherChanges changes)
    {
        ...
        else
        {
            switch (patch.path)
            {
                ...
                default: Logger.LogVerbose($"Not implemented remove patch with path[{patch.path}]"); break;
            }
        }
    }
