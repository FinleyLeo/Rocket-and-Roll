...
namespace Unity.Services.Multiplayer.Editor.Matchmaker.Authoring.UI
{
    ...
    class MatchmakerQueueInspector : AssetImporterEditor
    {
            ...
            for (int i = 0; i < targets.Length; i++)
            {
                ...
                if (m_Configs.TryGetValue(i, out var config))
                {
                    if (extraDataTargets[i] is MatchMakerQueueInspectorConfig editedConfig)
                    {
                        ...
                        m_Configs[i] = currentObj.ToObject<QueueConfig>(JsonSerializer.Create(SerializerSettings));
                        string json =
                            JsonConvert.SerializeObject(m_Configs[i], Formatting.Indented, SerializerSettings);
                        File.WriteAllText(matchmakerFilePath, json);
                    }
                }
            }
            RevertInspectorDataWithCurrentConfig();
        }
        ...
        private void RevertInspectorDataWithCurrentConfig()
        {
            ...
            for (int i = 0; i < targets.Length; i++)
            {
                if (m_Configs.TryGetValue(i, out var config))
                {
                    if (extraDataTargets[i] is MatchMakerQueueInspectorConfig serializedConfig)
                    {
                        serializedConfig.Initialize(config);
                    }
                }
            }
